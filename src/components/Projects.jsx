...
export const Projects = () => {
  ...
  return (
    <section id="Projects" className="min-h-screen bg-gradient-to-br from-sky-50 via-sky-100 to-white py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto w-full">
        {/* Header */}
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-3 sm:mb-4">
            My <span className="bg-gradient-to-r from-tealbrand-500 via-cyan-500 to-tealbrand-600 bg-clip-text text-transparent">Projects</span>
          </h2>
          <p className="text-sm sm:text-base md:text-lg text-gray-700 max-w-xs sm:max-w-md md:max-w-lg lg:max-w-2xl mx-auto">
        ...
        <div className="relative">
          ...
          <div ref={scrollRef} className="flex gap-6 overflow-x-auto pb-6 scrollbar-thin scroll-smooth">
            {projects.map((project, index) => (
              <div 
                key={index}
                className={`min-w-[350px] sm:min-w-[400px] lg:min-w-[450px] bg-white rounded-xl p-6 transition-all duration-500 transform hover:scale-105 border border-black/10 hover:border-brand-500/40 hover:shadow-glow cursor-pointer ${
                  selectedProject === index ? 'scale-110 z-10' : ''
                }`}
                onClick={() => setSelectedProject(selectedProject === index ? null : index)}
              >
                {/* Project Image */}
                <div className="w-full h-48 bg-gradient-to-br from-tealbrand-500/10 to-cyan-600/10 rounded-lg mb-4 overflow-hidden border border-black/10">
                  <img 
                    src={project.image} 
                    alt={project.title}
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies.map((tech) => (
                    <span 
                      key={tech}
                      className="px-3 py-1 bg-black/5 text-tealbrand-700 text-xs rounded-full border border-black/10"
                    >
                      {tech}
                    </span>
                  ))}
        ...
        <div className="text-center mt-12 sm:mt-16">
          ...
          <button 
            ...
            className="px-6 sm:px-8 py-3 bg-gradient-to-r from-tealbrand-500 to-cyan-600 text-white font-semibold rounded-lg hover:from-tealbrand-600 hover:to-cyan-700 transition-all duration-300 transform hover:scale-105 text-sm sm:text-base cursor-pointer shadow-glow"
          >
            Get In Touch
          </button>
        </div>
